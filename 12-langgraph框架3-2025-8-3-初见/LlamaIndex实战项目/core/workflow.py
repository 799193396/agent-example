...
class RAGWorkflow(Workflow):
    ...
    async def retrieve_step(
        ...
        """手动并行执行BM25和向量检索的混合检索"""
        # 并行执行两个检索器
        vector_task = asyncio.create_task(self.retriever.aretrieve(ev.query))
        bm25_task = asyncio.create_task(self.BM25_retriever.aretrieve(ev.query))

        # 并发等待两个检索完成
        vector_nodes, bm25_nodes = await asyncio.gather(vector_task, bm25_task)

        logger.info(f"向量检索获得 {len(vector_nodes)} 个节点")
        logger.info(f"BM25检索获得 {len(bm25_nodes)} 个节点")

        return RAGEvents.RetrievalEvent(query=ev.query, nodes=vector_nodes + bm25_nodes)
